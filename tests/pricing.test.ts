...
describe("server panel pricing", () => {
  it("keeps every fixed panel position and an exact v1 price", () => {
    expect(PANEL_PRICES.map((item) => item.size)).toEqual([20, 30, 35, 50, 75, 100, 150, 250]);
    expect(quotePanel(20)).toEqual({ size: 20, priceCents: 500, discountPercent: 0, priceVersion: PRICE_VERSION });
    expect(quotePanel(250).priceCents).toBe(4_000);
  });
  ...
  it("applies increasing visible discounts", () => {
    expect(PANEL_PRICES.map((item) => item.discountPercent)).toEqual([0, 4, 6, 10, 15, 20, 28, 36]);
  });
});
