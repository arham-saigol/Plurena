...
function Responses({ data, queryStatus, loadMore }: { data: ResponsesData; queryStatus: PaginationStatus; loadMore(): void }) {
  ...
  return <div className="response-list">{sorted.map((response) => <article className="response-card" key={response._id}><div className="persona-line"><div className="persona-avatar">{response.persona.ordinal + 1}</div><div><strong>{response.persona.age}, {response.persona.location}</strong><span>{response.persona.gender} · {response.persona.pointOfView}</span></div></div><div className="response-answer"><span>{data.test.testType === "compare" ? "Choice" : "Answer"}</span><p>{data.test.testType === "compare" ? (optionMap.get(String(response.choiceOptionId))?.label ?? "None of the above") : response.answer}</p></div><ul className="feedback-list">{response.feedback.map((item, index) => <li key={index}>{item}</li>)}</ul><div className="persona-tags">{response.persona.interests.map((item) => <span key={item}>{item}</span>)}{response.persona.constraints.map((item) => <span key={item}>{item}</span>)}</div></article>)}{queryStatus === "CanLoadMore" && <button className="button secondary" onClick={loadMore}>Load more responses</button>}</div>;
}
