...
export function TestDetails({ testId }: { testId: string }) {
  ...
  return <div className="app-shell"><AppHeader /><main className="details-main">
    ...
    {running && <Card className="live-progress"><div aria-live="polite"><span className="live-dot" /><strong>{test.status === "synthesizing" ? "Responses complete. Plurena is writing the synthesis." : test.completedCount ? "Responses are arriving." : "The panel is preparing responses."}</strong><span>{test.completedCount} completed · {test.failedCount} failed</span></div><b>{progress}%</b><Progress value={progress} aria-label="Panel progress" aria-valuemin={0} aria-valuemax={test.panelSize} aria-valuenow={test.completedCount + test.failedCount} /></Card>}
    ...
    {test.status === "failed" && <Alert role="status" variant="destructive"><WarningCircle size={18} /><AlertTitle>No usable responses</AlertTitle><AlertDescription>Each assigned model and fallback failed. Your setup remains available for a rerun.</AlertDescription></Alert>}
...
function Responses({ data, queryStatus, loadMore }: { data: ResponsesData; queryStatus: PaginationStatus; loadMore(): void }) {
  ...
  return <div className="response-list">{sorted.map((response) => <Card className="response-card" key={response._id}><div className="persona-line"><div className="persona-avatar">{response.persona.ordinal + 1}</div><div><strong>{response.persona.age}, {response.persona.location}</strong><span>{response.persona.gender} · {response.persona.pointOfView}</span></div></div><div className="response-answer"><span>{data.test.testType === "compare" ? "Choice" : "Answer"}</span><p>{data.test.testType === "compare" ? (optionMap.get(String(response.choiceOptionId))?.label ?? "None of the above") : response.answer}</p></div><ul className="feedback-list">{response.feedback.map((item, index) => <li key={index}>{item}</li>)}</ul><div className="persona-tags">{response.persona.interests.map((item) => <Badge variant="secondary" key={item}>{item}</Badge>)}{response.persona.constraints.map((item) => <Badge variant="outline" key={item}>{item}</Badge>)}</div></Card>)}{queryStatus === "CanLoadMore" && <Button variant="outline" onClick={loadMore}>Load more responses</Button>}</div>;
}
...
function createMarkdown(data: ResponsesData) {
  ...
  for (const response of responses.filter(hasPersona)) lines.push(`### Respondent ${response.persona.ordinal + 1}: ${response.persona.age}, ${response.persona.location}`, "", test.testType === "compare" ? `Choice: ${optionMap.get(String(response.choiceOptionId)) ?? "None of the above"}` : (response.answer ?? ""), "", ...response.feedback.map((item) => `- ${item}`), "");
  return lines.join("\n");
}
