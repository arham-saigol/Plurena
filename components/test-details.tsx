...
export function TestDetails({ testId }: { testId: string }) {
  ...
  return <div className="app-shell"><AppHeader /><main className="details-main">
    ...
    <section className="result-header"><div><div className="result-kicker"><span className={`status ${test.status}`}><i />{test.status}</span><span>{timeAgo(test.launchedAt, renderedAt)}</span></div><h1>{test.title}</h1><p>{test.testType === "compare" ? "Comparison" : "Open question"} · {test.panelSize} respondents · {money(test.priceCents)}</p></div><div className="header-stat"><span>Responses</span><strong>{test.completedCount}<small>/{test.panelSize}</small></strong></div></section>
...
function Responses({ data, queryStatus, loadMore }: { data: ResponsesData; queryStatus: PaginationStatus; loadMore(): void }) {
  ...
  return <div className="response-list">{sorted.map((response) => <article className="response-card" key={response._id}><div className="persona-line"><div className="persona-avatar">{response.persona.ordinal + 1}</div><div><strong>{response.persona.age}, {response.persona.location}</strong><span>{response.persona.gender} · {response.persona.pointOfView}</span></div><small>{response.provider.replace("_", " ")} · {response.model}</small></div><div className="response-answer"><span>{data.test.testType === "compare" ? "Choice" : "Answer"}</span><p>{data.test.testType === "compare" ? (optionMap.get(String(response.choiceOptionId))?.label ?? "None of the above") : response.answer}</p></div><ul className="feedback-list">{response.feedback.map((item, index) => <li key={index}>{item}</li>)}</ul><div className="persona-tags">{response.persona.interests.map((item) => <span key={item}>{item}</span>)}{response.persona.constraints.map((item) => <span key={item}>{item}</span>)}</div></article>)}{queryStatus === "CanLoadMore" && <button className="button secondary" onClick={loadMore}>Load more responses</button>}</div>;
}
