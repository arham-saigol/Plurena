...
export function NewTestDialog({ pricing, balanceCents, onClose, seed }: { pricing: Pricing; balanceCents: number; onClose(): void; seed?: RerunSeed }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) close(); }}><DialogContent className="grid max-h-[min(92svh,820px)] w-[calc(100%-1.5rem)] max-w-3xl grid-rows-[auto_auto_minmax(0,1fr)_auto_auto] gap-0 overflow-hidden rounded-2xl border bg-background p-0 text-foreground shadow-2xl ring-0 sm:max-w-3xl" showCloseButton>
    ...
    <ol className="grid grid-cols-5 border-b bg-muted/25 px-3 sm:px-6" aria-label="Test creation progress">{steps.map((label, index) => <li key={label} className={cn("relative flex min-w-0 items-center justify-center gap-2 py-3 text-[11px] font-medium text-muted-foreground after:absolute after:bottom-0 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-foreground after:transition-all", index === step && "text-foreground after:w-full", index < step && "text-foreground")} aria-current={index === step ? "step" : undefined}><span className={cn("grid size-5 shrink-0 place-items-center rounded-full border bg-background font-mono text-[9px]", index <= step && "border-foreground text-foreground", index < step && "bg-foreground text-background")}>{index < step ? <Check size={10} weight="bold" /> : index + 1}</span><span className="hidden truncate sm:inline">{label}</span></li>)}</ol>
    ...
    <div className="flex items-center justify-between gap-3 border-t bg-muted/25 px-5 py-4 sm:px-7"><Button variant="ghost" disabled={step === 0 || busy} onClick={() => { setError(""); setStep((value) => value - 1); }}><ArrowLeft size={15} /> Back</Button>{step < 4 ? <Button className="min-w-28 bg-foreground text-background hover:bg-foreground/90" disabled={!valid || busy} onClick={() => { setError(""); setStep((value) => value + 1); }}>Continue <ArrowRight size={15} /></Button> : <Button className="min-w-40 bg-foreground text-background hover:bg-foreground/90" disabled={busy || balanceCents < quote.priceCents} onClick={submit}>{busy ? "Launching…" : `Launch for ${money(quote.priceCents)}`} <ArrowRight size={15} /></Button>}</div>
