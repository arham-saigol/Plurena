...
type OptionDraft = { key: string; label: string; optionType: "text" | "image"; text: string; assetId?: string; fileName?: string };
...
export type RerunSeed = { testId: string; title: string; testType: "compare" | "question"; options: OptionDraft[]; audience: Audience; panelSize: number; reusePanel?: boolean };
...
export function NewTestDialog({ pricing, balanceCents, onClose, seed }: { pricing: Pricing; balanceCents: number; onClose(): void; seed?: RerunSeed }) {
  ...
  const upload = async (index: number, file?: File) => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Image upload failed."); } finally { setBusy(false); }
  };
  const storeAudience = async () => {
    if (!saveName.trim()) { setError("Name this audience first."); return; }
    setBusy(true); setError("");
    try { await saveAudience({ name: saveName, criteria: { ...audience, name: saveName } }); setSaveName(""); } catch (cause) { setError(cause instanceof Error ? cause.message : "Audience could not be saved."); } finally { setBusy(false); }
  };
  const submit = async () => {
    ...
    try {
      const result = await launch({
        ...
        title,
        testType,
        options: options.map(({ label, optionType, text, assetId }) => ({ label, optionType, text: text || undefined, assetId: assetId as any })),
        audience,
        panelSize: quote.size,
        rerunOf: seed?.testId as any,
        reusePanel: seed ? reusePanel : false,
      });
      router.push(`/tests/${result.testId}`);
    } catch (cause) { setError(readError(cause)); setBusy(false); }
  };
...
function ContentStep({ testType, title, setTitle, options, setOptions, updateOption, addOption, removeOption, upload, busy }: any) {
...
function AudienceStep({ audience, setAudience, saved, saveName, setSaveName, save }: any) {
  const genderIndex = audience.gender === "female" ? 0 : audience.gender === "mixed" ? 1 : 2;
  return <div className="form-stack">{saved.length > 0 && <label className="field"><span>Saved audience</span><select defaultValue="" onChange={(event) => { const match = saved.find((item: any) => item._id === event.target.value); if (match) setAudience(match.criteria); }}><option value="">Choose saved criteria</option>{saved.map((item: any) => <option key={item._id} value={item._id}>{item.name}</option>)}</select></label>}
    ...
    <div className="range-field"><div><span>Gender mix</span><b>{genderLabels[genderIndex]}</b></div><input type="range" aria-label="Gender mix" aria-valuetext={genderLabels[genderIndex]} min="0" max="2" step="1" value={genderIndex} onChange={(event) => setAudience({ ...audience, gender: ["female", "mixed", "male"][Number(event.target.value)] })} /><div className="range-labels"><span>Female</span><span>Mixed</span><span>Male</span></div></div>
...
function readError(cause: unknown) {
  const message = cause instanceof Error ? cause.message : "The test could not launch.";
  if (message.includes("INSUFFICIENT_CREDIT")) return "Your balance is too low for this panel. Add balance and try again.";
  return message.replace(/^\[CONVEX [^\]]+\]\s*/, "");
}
